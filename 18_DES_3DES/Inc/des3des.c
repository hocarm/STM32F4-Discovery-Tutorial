...
void key_schedule(uchar key[], uchar schedule[][6], uint mode)
{
   ...
   // Permutated Choice #1 (copy the key in, ignoring parity bits).hoan vi lua chon(sao chep khoa trong bo qua bit chan le)
   for (i = 0, j = 31, C = 0; i < 28; ++i, --j)
      C |= BITNUM(key,key_perm_c[i],j);
   for (i = 0, j = 31, D = 0; i < 28; ++i, --j)
      D |= BITNUM(key,key_perm_d[i],j);

   // Generate the 16 subkeys. tao ra 16 khoa
   for (i = 0; i < 16; ++i) {
      ...
      if (mode == DECRYPT)
         ...
         to_gen = i;
      // Initialize the array khoi tao mang
      for (j = 0; j < 6; ++j)
         schedule[to_gen][j] = 0;
      for (j = 0; j < 24; ++j)
...
uint f(uint state, uchar key[])
{
   ...
   // Expantion Permutation hoan vi mo rong 
